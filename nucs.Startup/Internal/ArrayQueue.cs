...
namespace nucs.Collections {
    ...
    [Serializable]
    public sealed class ArrayQueue<T> : IEnumerable<T>, ICollection {
        ...
        public ImprovedList<T> ToImprList() {
            return new ImprovedList<T>(ToArray());
        }
        ...
        public bool Remove(T item) {
            if (_size == 0)
                return false;
            var s = new ImprovedList<T>(ToArray());
            if (s.Remove(item)) {
                _array = s.ToArray();
                --_tail;
                --_size;

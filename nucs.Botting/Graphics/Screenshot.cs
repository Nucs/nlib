...
namespace nucs.Windows.Graphics {
    public static class Screenshot {
        /// <summary>
        /// Takes a SS and saves it to <paramref name="filename"/>
        /// </summary>
        public static void TakeAndSave(string filename) {
        ...
        private static int GetGDIHandleCount() { return GetGuiResources(Process.GetCurrentProcess().Handle, 0); }

        private static int GetUserHandleCount() { return GetGuiResources(Process.GetCurrentProcess().Handle, 1); }

        private static void HandleMessage(string message) { Debug.WriteLine("HC: " + message + ": GDI: " + GetGDIHandleCount().ToString() + ": User: " + GetUserHandleCount().ToString()); }

...
{
    [TestClass]
    public class Database_ExecuteEntities
    {
        [TestMethod]
        public void ExecuteEntities()
        {
            ...
            {
                List<TestObject> list = ctx.Database.ExecuteEntities<TestObject>(sql, dict.ToDbParameters(ctx.Database.Connection)).ToList();

...
namespace nucs.Windows.Keyboard.CardReader {
    ...
    public sealed class InputDevice {
        ...
        private static string ReadReg(string item, ref bool isKeyboard) {
            ...
            var findme = string.Format(@"System\CurrentControlSet\Enum\{0}\{1}\{2}", id_01, id_02, id_03);
            ...
            if (deviceClass.ToUpper().Equals("KEYBOARD")) isKeyboard = true;
        ...
        public int EnumerateDevices() {
            ...
            if (GetRawInputDeviceList(IntPtr.Zero, ref deviceCount, (uint) dwSize) == 0) {
                ...
                for (int i = 0; i < deviceCount; i++) {
                    ...
                    if (pcbSize > 0) {
                        ...
                        if (deviceName.ToUpper().Contains("ROOT")) continue;
            ...
            else throw new ApplicationException("An error occurred while retrieving the list of devices.");
        }
            ...
            try {
                ...
                if (buffer != IntPtr.Zero &&
                    ...
                    if (raw.header.dwType == RIM_TYPEKEYBOARD) {
                        ...
                        //if ("1234567890".Any(c=>c.ToString() == raw.keyboard.Message.ToString()))
                        if (true) {
                            ...
                            if (raw.header.hDevice == IntPtr.Zero) {
                                ...
                                goto _machineCodeInput;
                            }
                            if (deviceList[raw.header.hDevice] == null) EnumerateDevices();
                            if ((dInfo = (DeviceInfo) deviceList[raw.header.hDevice]) == null) {
                                throw new ApplicationException("Error detecting the device in 'deviceList' by id of:" +
                                                               raw.header.hDevice);
                            }
                            _machineCodeInput:
                            ...
                            if (KeyPressed != null && dInfo != null)
                                ...
                                string errMessage =
                                    String.Format("Received Unknown Key: {0}. Possibly an unknown device", key);
        ...
        #region GetKeyboardsList()

        public static List<DeviceInfo> GetKeyboardsList() {
            return new List<DeviceInfo>(GetKeyboardsDictionary().Values);
        }

        public static IDictionary<IntPtr, DeviceInfo> GetKeyboardsDictionary() {
            IDictionary<IntPtr, DeviceInfo> list = new Dictionary<IntPtr, DeviceInfo>();
            int NumberOfDevices = 0;
            uint deviceCount = 0;
            int dwSize = (Marshal.SizeOf(typeof (RAWINPUTDEVICELIST)));
                ...
                for (int i = 0; i < deviceCount; i++) {
                    ...
                    if (pcbSize > 0) {
                        ...
                        if (rid.dwType == RIM_TYPEKEYBOARD || rid.dwType == RIM_TYPEHID) {
                            ...
                            // add it to the deviceList hashtable and increase the
                            // NumberOfDevices count
                            if (!list.ContainsKey(rid.hDevice) && IsKeyboardDevice) {
                                NumberOfDevices++;
                                list.Add(rid.hDevice, dInfo);
                            }
                        }
                        Marshal.FreeHGlobal(pData);
                    }

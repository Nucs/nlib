...
public static partial class DirectoryInfoExtension
{
    ...
    {
        return Directory.GetFiles(@this.FullName).Select(x => new DirectoryInfo(x)).Where(x => predicate(x)).ToArray();
    }
    ...
    {
#if (NET35 || NET3 || NET2)
        return Directory.GetDirectories(@this.FullName, searchPattern).Select(x => new DirectoryInfo(x)).Where(x => predicate(x)).ToArray();
#else
    ...
    {
        return Directory.GetDirectories(@this.FullName, searchPattern, searchOption).Select(x => new DirectoryInfo(x)).Where(x => predicate(x)).ToArray();
    }

...
namespace TFer.Filesystem.Distribution {
    public static class AIFileNameGenerator {
        ...
        public static FileInfo Generate(DirectoryInfo @base) {
            ...
            if (files.Where(f=> f.HasExtension() && blacklisted_extension.Any(ext=>ext.Equals(f.Extension))==false).Where(f=>!string.IsNullOrEmpty(f.GetFileNameWithoutExtension())).Any(f => f.GetFileNameWithoutExtension().Reverse().First().IsDigit())) {
                var potent = files.Where(f => f.HasExtension() && blacklisted_extension.Any(ext => ext.Equals(f.Extension))==false).Where(f => !string.IsNullOrEmpty(f.GetFileNameWithoutExtension()) && f.GetFileNameWithoutExtension().Reverse().First().IsDigit()).OrderBy(f=>f.LastAccessTimeUtc).FirstOrDefault(); //get least touched file with number at end.
                if (potent != null) {
        ...
        public static string FavoriteExtension(IEnumerable<FileInfo> filessample, params string[] except) {
            ...
            if (files.Length <= 1) goto _rand; //empty or one file
            ...
            if (string.IsNullOrEmpty(logical)==false) return logical;

            _rand:
            return datastorageextensions.Except(except).TakeRandom();
        }

        public static T TakeRandom<T>(this IEnumerable<T> list) {return TakeRandom(list.ToList());}

...
namespace BotSuite.ImageLibrary {
    ...
    public class ImageData {
        ...
        public ImageData(Bitmap Bitmap) { LoadBitmap(Bitmap); }
        ...
        public ImageData(string Path) { LoadBitmap(new Bitmap(Path)); }
        ...
        /// </example>
        /// <param name="Path">path to bitmap</param>
        public ImageData(ImageData Img) { LoadBitmap(Img.Bitmap); }
        ...
        public void Save(string Path) { this.Bitmap.Save(Path); }
        ...
        public ImageData Clone() { return new ImageData(this); }
        ...
        /// <param name="H">height of imagepart (default: full height)</param>
        /// <returns></returns>
        public Bitmap Crop(int L = 0, int T = 0, int W = -1, int H = -1) {
            // First, we generate a Bitmap object holding the current bitmap (ReturnBitmap)
            W = (W == -1) ? Width - L : W;
            H = (H == -1) ? Height - T : H;
        ...
        public Color this[int x, int y] {
            get {
                if ((0 <= x) && (x < Width) && (0 <= y) && (y < Height)) {
                    return Color.FromArgb(BmpBytes[y*BmpStride + x*RawFormatOffset + 2], BmpBytes[y*BmpStride + x*RawFormatOffset + 1], BmpBytes[y*BmpStride + x*RawFormatOffset]);
                } else {
                    return Color.Empty;
                }
            }
        ...
        /// <param name="y">y coordinate (row)</param>
        /// <returns>color of pixel</returns>
        public Color GetPixel(int x, int y) { return this[x, y]; }
        ...
        public void SetPixel(int row, int col, Color Color) { this[row, col] = Color; }
